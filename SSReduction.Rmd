...
#ODE for a three-pool FOLD model
...
#initial value
InitialCarbon_3pool <- C_ss_3feedback(parms = century_parameter.ls)*0.75
...
sim_Century <- lsoda(y = c(G=0, 
                              #C_fast = 0, C_slow = 0, C_passive =0), #use for full range of carbon stocks comparison?
                              InitialCarbon_3pool),
                      times = 0:simTime,
                      func = dCdt_3feedback,
...
InitialCarbon_1pool <- sum(C_ss_3feedback(parms = century_parameter.ls))*0.75
...
sim_Century <- sim_Century %>% 
                  ...
                  mutate(dCO2dt = dCdt_3feedback(t=NA, y=c(G=0,C_fast, C_slow, C_passive), parms= century_parameter.ls)[[1]]["G"], .by=everything() )
...
No_inputs_feedback_sim <- lsoda(y = c(G=0, C_ss_3feedback(parms = century_parameter.ls)),
                                      times = Simtimes,
                                      func = dCdt_3feedback,
                          ...
                          mutate(C_total = C_fast + C_slow +C_passive) %>%
                          mutate(model="Century") %>%
                          mutate(dCO2dt = dCdt_3feedback(t=NA, y=c(G=0,C_fast, C_slow, C_passive), parms= no_input_century_parameter.ls)[[1]]["G"], .by=everything() )
                

No_inputs_one_pool_proxy_sim <- lsoda(y = c(G=0, C_total=sum(C_ss_3feedback(parms = century_parameter.ls))),
                                      times = Simtimes,
                                      func = dCdt_1pool,
                                      parms = no_input_century_parameter.ls) %>%
                                  as.data.frame() %>%
...
seasonal_century.sim <- lsoda(y = C_ss_3feedback(parms = century_parameter.ls),
                                      times = Simtimes, 
                                  ...
                                  mutate(C_total = C_fast+C_slow+C_passive) %>%
                                  mutate(model= "Century") %>%
                                  mutate(dCO2dt = dCdt_3feedback(t=NA, 
...
seasonal_proxy.sim <- lsoda(y = sum(C_ss_3feedback(parms = century_parameter.ls)) %>% setNames("C"),
                                      times = Simtimes, 
...
regime_change_feedback_sim <- lsoda(y = c(C_ss_3feedback(parms = century_parameter.ls), Inputs = 0, G=0),
                                      times = Simtimes,
...
regime_change_proxy_sim <- lsoda(y = c(C_total=sum(C_ss_3feedback(parms = century_parameter.ls)), Inputs=0, G=0),
                                      times = Simtimes,
...
detrital_addition_feedback_sim <- lsoda(y = c(C_ss_3feedback(parms = century_parameter.ls), G=0),
                                      times = Simtimes,
...
detrital_addition_proxy_sim <- lsoda(y = c(C_total=sum(C_ss_3feedback(parms = century_parameter.ls)), G=0),
                                      times = Simtimes,
...
century_parameter5.ls <- list(inputs = 0.2/12, #monthly; divided evenly between the months
                            input_to_struc = .125,  
                            input_to_meta = .875,  
