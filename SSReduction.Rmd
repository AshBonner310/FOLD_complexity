...
dCdt_1pool <- function(t, y, parms, rel_tol = 1e-8){
  ...
  #if(abs(parms$inputs - (dCO2 + dSoil))/parms$inputs > rel_tol){ #This is causing the no-inputs run to fail.
  if(abs(parms$inputs - (dCO2 + dSoil)) > rel_tol){
...
No_inputs_feedback_sim <- rbind(lsoda(y = c(G=0, C_ss_3feedback(parms = century_parameter.ls)),
                                      times = 0:240,
                                      func = dCdt_3feedback,
                                      parms = century_parameter.ls) %>%
                                  as.data.frame(),
                              lsoda(y = c(G=4, C_ss_3feedback(parms = century_parameter.ls)),
                                      #Tried this, it ran but it's clearly no giving me the right number for respiration integration, which I guess makes sense but I'm bummed about it: unlist(dCdt_3feedback(t=240,y=c(G=0, C_ss_3feedback(parms = century_parameter.ls)), parms = century_parameter.ls)), 
                                      times = 0:9760,
                                      func = dCdt_3feedback,
...
#Highlighting the change in respiration
ggplot(No_inputs_feedback_sim) +
  geom_line(aes(x=time, y=G)) +
  geom_vline(aes(xintercept=240), linetype=2) +
  xlim(0,480) +
  geom_abline(aes(intercept=0, slope=4/240), color='red', alpha=0.5)
                


#Simulation with the one-pool model starting at SS, turn off inputs at time 240 (twenty years, monthly time steps)
No_inputs_one_pool_proxy_sim <- rbind(lsoda(y = c(0, sum(C_ss_3feedback(parms = century_parameter.ls))),
                                            times = 0:240,
                                            func = dCdt_1pool,
                                            parms = century_parameter.ls) %>%
                                       as.data.frame() %>%
                                       set_names(c("time", "Respiration", "One_Pool_Proxy")),
                                      
                                    lsoda(y = c(4, sum(C_ss_3feedback(parms = century_parameter.ls))),
                                            times = 0:9760,
                                            func = dCdt_1pool,
                                            parms = no_input_century_parameter.ls) %>%
                                      as.data.frame() %>%
...
ggplot(No_inputs_one_pool_proxy_sim) +
  geom_line(aes(x=time, y=Respiration)) +
  geom_vline(aes(xintercept=240), linetype=2) +
  xlim(0,480) +
  geom_abline(aes(intercept=0, slope=4/240), color='red', alpha=0.5)
...
ggplot(no_input_model_comparison) +
  geom_point(aes(x=Feedback_C_Total, y=One_Pool_Proxy)) +
  geom_abline()



ggplot(no_input_model_comparison %>% pivot_longer(cols = c("Feedback_C_Total", "One_Pool_Proxy"),
                                 names_to = "model",
                                 values_to = "stock"))  +
