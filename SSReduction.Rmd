...
dCdt_1pool <- function(t, y, parms){
  ...
  if(abs(parms$inputs - (dCO2 + dSoil))/parms$inputs > 1e-8){
...
ggplot(sim_1pool %>%
         pivot_longer(cols = c(C, G), names_to = 'pool', values_to = 'mass')) +
...
sim_1pool <- sim_1pool %>% 
  ...
  mutate(cumulativeInputs = time * parameter.ls$inputs) %>%
  as.data.frame() %>%
  mutate(balanceC = C[1] + cumulativeInputs - (RespiredC + C))
