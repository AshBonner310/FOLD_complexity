...
```{r}
 #u; [0, 2.5] kg-carbon m-2 yr-1; evenly split per month for now
...
InitialCarbon_1pool <- C_ss_1pool(parms = parameter.ls)*0.7  %>% setNames("C")
...
sim_1pool <- lsoda(y = InitialCarbon_1pool,
                times = 0:simTime,
                func = dCdt_1pool,
                parms = parameter.ls) %>%
  as.data.frame() 
...
At any point in time, the rate of gaseous CO$_2$ respired from the soil is

$$\frac{dG}{dt} = kC$$

and the total CO$_2$ respired from time $t=0$ to $t=T$ is

$$G(t) = \int_0^T kC(t) dt$$

This gives us good motivation to solve the seperable ordinary differential equation for the one-pool model above.

$$\frac{dC}{dt} = u - kC \implies C(t) = \frac{u}{k} - Ae^{-kt} $$

where $A$ is a stand-in for some constant.
We can find this constant by calling the initial value of the system $C_0$, such that $C(t=0)=C_0$.

$$C(0)=C_0 = \frac{u}{k} - Ae^k(0) = \frac{u}{k}-A \implies A= \frac{u}{k}-C_0$$

This means we can develop an expression for how much gaseous CO$_2$ has respired from the soil:

$$G(T) = \int_0^T k\left( \frac{u}{k} - \left(\frac{u}{k}-C_0\right)e^{-kt}\right) dt = uT + k \left( u-kC_0 \right)\left(1-e^{-kT}\right)$$
Let's apply this to our simulation above.

```{r}

RespiredCarbon <- function(endtime, parms, initialstate) {
  G = parms$inputs*endtime +
      ((parms$inputs - initialstate/parms$turnoverTime)/parms$turnoverTime)*(1-exp(-endtime/parms$turnoverTime))
  return(G)
}

sim_1pool <- sim_1pool %>% 
  mutate(RespiredC = RespiredCarbon(endtime=time, parms=parameter.ls, initialstate=InitialCarbon_1pool) ) %>%
  as.data.frame()

 ggplot(sim_1pool) +
  geom_line(aes(x=time, y=C)) +
  geom_area(aes(x=time, y=RespiredC), alpha=0.5)
...
dCdt_3feedback <- function(t, y, parms){
 ...
 allocation_matrix <- matrix(c(parms$input_to_fast, 
  ...
  ##TRYING TO ADD RESPIRATION HERE AND IT AIN'T WORKIN##

  
  # dgasdt <- parms$turnoverTime_fast*(1- parms$fast_to_slow - parms$fast_to_passive)*ans[1]  +
  #         parms$turnoverTime_slow*(1- parms$slow_to_fast - parms$slow_to_passive)*ans[2]  +
  #         parms$turnoverTime_passsive*(1- parms$passive_to_fast - parms$passive_to_slow)*ans[3] 
...
InitialCarbon_1pool <- sum(C_ss_3feedback(parms = century_parameter.ls))*0.6 %>% setNames("C")

#Simulation with the one-pool model
sim_Feedback_OnePool <- lsoda(y = InitialCarbon_1pool,
                times = 0:simTime,
                func = dCdt_1pool,
                parms = century_parameter.ls) %>%
  as.data.frame()
