...
dCdt_OnePool.fn <- function(t, y, parms, rel_tol = 1e-8){ #
  ...
  if(parms$input_type == 'seasonal'){
      ...
      inputs <- 0
        ...
        if(! all(c('shift_percent', 'shift_time') %in% names(parms))){
        stop('You have a parameter missing that this function needs.')
          }
      if(t > parms$shift_time ){
...
Parameters_FivePool.ls <- list(
                    input_type = 'base', #options: seasonal, incubation, regime shift
...
ggplot(Season_Proxy_sim.df) + 
  geom_line(aes(x=time, y=RespiratonRate))
...
Parameters_FivePool.ls$shift_time <- 12 #regime shift occurs one year into simulation
