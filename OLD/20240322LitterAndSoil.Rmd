...
SpinUp_FivePool_sim.df <- lsoda(y = c(Cummulative_Respiration=0, 
                            SS_FivePool.fn(parms = Parameters_FivePool.ls)*0),
...
SpinUp_Proxy_sim.df <- lsoda(y = c(Cummulative_Respiration=0, 
                                    C_total = sum(SS_FivePool.fn(parms=Parameters_FivePool.ls))*0),
                                    times = FivePoolSimTime,
                                    func = dCdt_OnePool.fn,
                                    parms = Parameters_FivePool.ls) %>%
                          as.data.frame() %>%
...
RegimeShiftSimTime <- c(0:12, seq(12.5, 12+(50*12), by=0.5))
...
RegimeInputs <- data.frame(time = RegimeShiftSimTime) %>%
                      mutate("Regime shift" = InputType.fn(parms=list(inputs = Parameters_FivePool.ls$inputs, input_type = 'regime shift', shift_percent = 1.5, shift_time=12), timepoint = time),
                          .by = everything())


SimulationInputs.df <- data.frame(time = ShortSimTime, 
                                "SpinUp" = InputType.fn(parms=list(inputs = Parameters_FivePool.ls$inputs, input_type = 'base'), timepoint = ShortSimTime),
...
ggplot(SimulationInputs.df %>% pivot_longer(cols='SpinUp':'Seasonal', names_to = "sim", values_to = "stock")) +
...
SpinUp_sim.df <- full_join(SpinUp_FivePool_sim.df %>%
  ...
  mutate(scenario = "SpinUp")


SpinUp_Stocks.gr <- ggplot(SpinUp_sim.df %>% filter(outcome == 'Total SOC') %>% filter(time < 12*100)) + 
  geom_line(aes(x=time/12, y=stock, linetype=sim)) +
  scale_linetype_manual(values=c("dotted", "twodash"))


SpinUp_Resp.gr <- ggplot(SpinUp_sim.df %>% filter(outcome == 'Soil Respiration') %>% filter(time < 12*10)) + 
...
SpinUp_SOC_comp.gr <- ggplot(SpinUp_sim.df %>% 
...
SpinUp_Resp_comp.gr <- ggplot(SpinUp_sim.df %>% 
         pivot_wider(names_from = sim,
                     values_from = stock) %>% 
...
SpinUp_SOC_comp.gr +
  labs(title="Total SOC Stocks Between Models", x="One Pool with Proxy Decay Rate", y="Five Pool First-Order Linear Decay") + SpinUp_Resp_comp.gr +
...
# temp.df <- Incubation_sim.df %>%
#       pivot_wider(names_from = sim,
#                   values_from = stock)

input_incub.df <- tibble(time = unique(Incubation_sim.df$time), 
                         outcome = "Inputs") %>%
                  mutate(stock = 0) %>% 
                  reframe(sim = c("FivePools", "Proxy"), 
                          .by = everything()) %>%
                  full_join(Incubation_sim.df) %>%
                  filter(outcome != "Total SOC")


ggplot(input_incub.df) + 
...
#difference drive graph for SOC
Incub_SOC_comp.gr <- ggplot(Incubation_sim.df %>% 
         pivot_wider(names_from = sim,
                     values_from = stock) %>% 
         filter(outcome == 'Total SOC')
       ) +
...
#difference drive graph for respiration
Incub_Resp_comp.gr <- ggplot(Incubation_sim.df %>% 
         pivot_wider(names_from = sim,
                     values_from = stock) %>% 
         filter(outcome == 'Soil Respiration')
       ) +
...
Incub_SOC_comp.gr +
  labs(title="Total SOC Stocks Between Models", x="One Pool with Proxy Decay Rate", y="Five Pool First-Order Linear Decay") + Incub_Resp_comp.gr +
  labs(title="Respiration Between Models", x="One Pool with Proxy Decay Rate", y="Five Pool First-Order Linear Decay")
...
Season_Stocks.gr<- ggplot(Seasonal_sim.df %>% 
                            filter(outcome == 'Total SOC') %>% 
                            filter(time <= 5*15)) + 
...
Season_Resp.gr<- ggplot(Seasonal_sim.df %>% 
                          filter(outcome == 'Soil Respiration') %>% 
                          filter(time <= 5*15)) + 
  geom_line(aes(x=time/12, 
                y=stock, 
...
Season_Resp.gr + 
    labs(title = "SeasonalRespiration Rates", x="Time (years)", y="kg-C per m^2 per month") + 
  plot_layout(guides = "collect", axis_titles = "collect")
...
```{r SeasonalModel2ModelComp}

#difference drive graph for SOC
Season_SOC_comp.gr <- ggplot(Seasonal_sim.df %>% 
         pivot_wider(names_from = sim,
                     values_from = stock) %>% 
         filter(outcome == 'Total SOC')
       ) +
...
#difference drive graph for respiration
Season_Resp_comp.gr <- ggplot(Seasonal_sim.df %>% 
         pivot_wider(names_from = sim,
                     values_from = stock) %>% 
         filter(outcome == 'Soil Respiration')
       ) +
...
Season_SOC_comp.gr +
  labs(title="Total SOC Stocks Between Models", x="One Pool with Proxy Decay Rate", y="Five Pool First-Order Linear Decay") + Season_Resp_comp.gr +
  labs(title="Respiration Between Models", x="One Pool with Proxy Decay Rate", y="Five Pool First-Order Linear Decay")
...
Parameters_FivePool.ls$shift_percent <- 1.5 #increases inputs by %50
...
if(all(RegimeShiftSimTime != c(0:Parameters_FivePool.ls$shift_time, seq(Parameters_FivePool.ls$shift_time+0.5, Parameters_FivePool.ls$shift_time+(10*12), by=0.5), seq((10*12)+10, (12*1000), by=12)) ) ){
  print('Your simulation vector does not match the one to create the input graph above.')
}


#simulation for Inbuation
Regime_FivePool_sim.df <- lsoda(y = c(Cummulative_Respiration=0, 
                            SS_FivePool.fn(parms = Parameters_FivePool.ls)),
                      times = RegimeShiftSimTime,
                      func = dCdt_FivePool.fn,
                      parms = Parameters_FivePool.ls) %>%
                  as.data.frame() %>%
                  mutate(C_total = Lit_metabolic + Lit_structural + Soil_fast + Soil_slow + Soil_passive) %>%
...
#Simulation with the one-pool model
Regime_Proxy_sim.df <- lsoda(y = c(Cummulative_Respiration=0, 
                                    C_total = sum(SS_FivePool.fn(parms=Parameters_FivePool.ls))),
                                    times = RegimeShiftSimTime,
                                    func = dCdt_OnePool.fn,
                                    parms = Parameters_FivePool.ls) %>%
...
RegimeShift_sim.df <- full_join(Regime_FivePool_sim.df %>%
  ...
  mutate(scenario = "Regime shift")


Regime_Stocks.gr <- ggplot(RegimeShift_sim.df %>% 
         filter(outcome == 'Total SOC')
       ) + 
  geom_line(aes(x=time, y=stock, linetype=sim)) +
  scale_linetype_manual(values=c("dotted", "twodash"))

Regime_Resp.gr <- ggplot(RegimeShift_sim.df %>% 
         filter(outcome == 'Soil Respiration')
       ) + 
  geom_line(aes(x=time, y=stock, linetype=sim)) +
  scale_linetype_manual(values=c("dotted", "twodash"))
...
#difference drive graph for SOC
Regime_SOC_comp.gr <- ggplot(RegimeShift_sim.df %>% 
         pivot_wider(names_from = sim,
                     values_from = stock) %>% 
         filter(outcome == 'Total SOC')
       ) +
...
#difference drive graph for respiration
Regime_Resp_comp.gr <- ggplot(RegimeShift_sim.df %>% 
         pivot_wider(names_from = sim,
                     values_from = stock) %>% 
         filter(outcome == 'Soil Respiration')
       ) +
...
Regime_SOC_comp.gr +
  labs(title="Total SOC Stocks Between Models", x="One Pool with Proxy Decay Rate", y="Five Pool First-Order Linear Decay") + Regime_Resp_comp.gr +
  labs(title="Respiration Between Models", x="One Pool with Proxy Decay Rate", y="Five Pool First-Order Linear Decay")
...
```{r Figure}
Figure.df <- SimulationInputs.df %>%
  pivot_longer(cols = c("SpinUp", "Incubation", "Seasonal", "Regime shift"),
               names_to = 'scenario',
               values_to = 'stock') %>%
  mutate(outcome = "Inputs") %>% 
  reframe(sim = c("FivePools", "Proxy"), 
          .by = everything()) %>%
  full_join(Incubation_sim.df) %>%
  full_join(SpinUp_sim.df) %>%
  full_join(Seasonal_sim.df) %>%
  full_join(RegimeShift_sim.df) 

Figure.df$scenario <- factor(Figure.df$scenario, levels = c("SpinUp", "Incubation", "Seasonal", "Regime shift"))


Fig1a <- ggplot(Figure.df %>%   
  filter(outcome != "Total SOC") %>%
  filter(time <= 120)) + 
    geom_line(aes(x=time/12, y=stock, linetype=sim)) +
    facet_grid(scenario~outcome, scales = "free") +
  theme_bw() + 
  labs(x = "Time (years)", y = "Rate (kg C per m^2 per month)", linetype = "Model Structure") + 
  plot_layout(axis_titles = "collect")


Fig1b <- ggplot(Figure.df %>%   
                  filter(outcome == "Total SOC") %>%
                  filter(time <= 120)) + 
    geom_line(aes(x=time/12, y=stock, linetype=sim)) +
    facet_grid(scenario~outcome, scales = "free") +
  theme_bw() + 
  labs(x = "Time (years)", y = "Total SOC Stock (kg C per m^2)", linetype = "Model Structure") 

Fig1a + Fig1b + plot_layout(guides = "collect", widths = c(2, 1)) 

ggsave(filename = file.path("Figures","MainFigure.png"))
...
temp <- Figure.df %>% 
         ...
         filter(outcome == "Total SOC") %>%
         #filter(time <= 150*12) %>%
         filter(stock >= 0.99*sum(SS_FivePool.fn(parms=Parameters_FivePool.ls)))
temp2 <- temp %>% filter(sim == "FivePools")

proxy_min <- min(temp$time)/12
fivepools_min <- min(temp2$time)/12

close_enough <- data.frame(sim = c("FivePools", "Proxy"),
                           percentage = c(fivepools_min, proxy_min))


Fig2 <- ggplot(Figure.df %>% 
         filter(scenario == "SpinUp") %>%
         filter(outcome == "Total SOC") %>%
         filter(time <= 500*12)) + 
  geom_line(aes(x=time/12, y=stock)) + 
  geom_vline(data = close_enough, mapping = aes(xintercept = percentage), linetype = "dotdash", alpha = 0.8, color = "blue") +
  facet_wrap(~sim,
             ncol = 1) + 
  theme_bw() + 
  labs(x = "Time (years)", y = "Total SOC Stock (kg C per m^2)", title = "Spin Up Speed")

ggsave(plot = Fig2,
       filename = file.path("Figures","SpinUpFig.png"))
