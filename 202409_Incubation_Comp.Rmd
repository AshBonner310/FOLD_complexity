...
##Spits out a graph for initial verification
...
#Runs forward simulation for five pool model
Incubation_Sim5.df <- lsoda(y = c("Cumulative_Respiration" = 0, 
  ...
  mutate(Soil_Carbon_Concentration = Fast_Soil + Slow_Soil + Passive_Soil) %>%
    mutate(Total_Carbon_Concentration = Metabolic_Plant_Residue + Structural_Plant_Residue + Fast_Soil + Slow_Soil + Passive_Soil) %>%
...
names(Incubation_Sim1.df)[names(Incubation_Sim1.df) == 'Soil_Carbon_Concentration'] <- 'Total_Carbon_Concentration'


IncubationSim.df <- full_join(Incubation_Sim1.df %>% 
...
ggplot(IncubationSim.df) + 
  geom_line(aes(x=time, y= Total_Carbon_Concentration, color= model)) + scale_x_log10() + scale_y_log10()

IncubationSim.df <- IncubationSim.df %>%
...
IncubSim_SOC_Diff.df <- IncubationSim.df %>%
...
ggplot(IncubSim_SOC_Diff.df) + 
  geom_line(aes(x=time, y=Norm_Difference))

#### LOKING AT RESPIRATION #####

IncubSim_Resp_Diff.df <- IncubationSim.df %>%
...
```{r Paper Incubation Graph}

temp_graph1 <- ggplot(IncubSim_Resp_Diff.df %>% 
                        select(c("time", "One_Pool", "Five_Pool")) %>%
                        pivot_longer(cols = One_Pool:Five_Pool,
                                     names_to = "model",
                                     values_to = "Resp")
                      ) + 
  geom_line(aes(x=time, y=Resp, color=model, linetype=model)) +
  labs(y= "Respiration",  #(g-C per m^2 per yr)
       color = "Model", 
       linetype = "Model",
       x = "Time (years)")+
  theme_bw()+
...
temp_graph2 <- ggplot(IncubSim_Resp_Diff.df) + 
  geom_line(aes(x=time, y=Difference)) + 
...
Incubation_respiration_graph <- temp_graph1 / temp_graph2 +
  plot_layout(axes = "collect",
              axis_titles = "collect",
              heights = c(5,2)) +
  plot_annotation(
...
ggsave("Figures/Incubation_Respiration.pdf",
        device = pdf, 
       width = 17.8,
       height = 6,
       units = "cm")

Incubation_respiration_graph

```

## Litter cohort
I'm curious to see what a litter cohort decays to using this method. Let's consider a chunk of litter - maybe 100 grams?

```{r}
##Smaller amount to decay initially, so we'll run the simulation for a shorter time.
SimTimes <- seq(0, 1e3, by = 1/12) #1000 years, monthly

##Runs forward simulation for one pool
Incubation_Sim1.df <- lsoda(y = c("Cumulative_Respiration" = 0, 
                                  "Soil_Carbon_Concentration" = 100),
                times = SimTimes,
                func = OnePool_ODE.fn,
                parms = Params_Aggregate.ls) %>%
  as.data.frame() %>%
...
#Runs forward simulation for five pool model
Incubation_Sim5.df <- lsoda(y = c("Cumulative_Respiration" = 0, 
                                  "Metabolic_Plant_Residue" = 100*Params_FivePool.df$input_to_meta,
                                  "Structural_Plant_Residue" = 100*Params_FivePool.df$input_to_struc,
                                  "Fast_Soil" = 0,
                                  "Slow_Soil" = 0,
                                  "Passive_Soil" = 0),
                times = SimTimes,
...
ggplot(Incubation_Sim5.df %>%
         mutate(Plant_Residue = Metabolic_Plant_Residue + Structural_Plant_Residue) %>%
         select(!c('Respiration_Rate', 'Input_Rate')) %>%
         pivot_longer(cols = Metabolic_Plant_Residue:Plant_Residue,
                                  names_to = 'Pool',
                                  values_to = 'Mass')) +
