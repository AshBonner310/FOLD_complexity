...
NPP_shifts <- c(0, 0.25, 0.5, 0.75, 0.9, 0.95, 1, 1.05, 1.1, 1.25, 1.5, 1.75, 2)
...
#   labs(title = "Regime Shift", subtitle = "Stacked Carbon Stocks and Rates by Pool", x = "Time (years)", y = "SOC (gC per m2) or Rate  (gC per m2 per year)")
# 
# 
# 
...
#          pivot_longer(cols = c('Fast_Soil', 'Slow_Soil', 'Passive_Soil', 'Plant_Residue'),
#                                   names_to = 'Pool',
#                                   values_to = 'Mass')) + 
#   geom_line(aes(x=time, y=Mass, color = inputs_shift)) +
#   facet_wrap(~ Pool, ncol=1, scales='free') + 
#   labs(title = "Regime Shift", subtitle = "Carbon Stocks by Pool", x = "Time (years)", y = "SOC (gC per m2)")
# 
# 
# ggplot(NPPshift_Sim5.df %>%
#          mutate(Plant_Residue = Metabolic_Plant_Residue + Structural_Plant_Residue) %>%
...
#                                   names_to = 'Pool',
#                                   values_to = 'Mass')) +
#   geom_line(aes(x=time, y=Mass, color = inputs_shift, linetype = Pool)) + 
#   labs(title = "Regime Shift: Five Pools", subtitle = "Carbon Stocks by Pool", x = "Time (years)", y = "Soil Carbon Concentration (gC per m2)")
...
ggplot(temp %>% 
         filter(Close == "TRUE") %>%
         mutate("NPP Shift" = as.numeric(inputs_shift))) + 
  geom_point(aes(x=time, y=`NPP Shift`, color = inputs_shift)) 
