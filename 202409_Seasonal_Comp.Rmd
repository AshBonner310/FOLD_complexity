...
Ave_SOC <- sum(SpunUp_SeasonalSim.df$Total_Carbon_Concentration)/(5*104*2 +2) #104 time points each year, for five years, across two model simulations, plus an extra two points due to keeping t=0 and t=5
...
temp_graph1 <- ggplot(temp_df) + 
  ...
  labs(y= "Soil Organic Carbon Concentration",  #(g-C per m^2 per yr)
       color = "Model", 
       linetype = "Model",
       x = "Time (years)")+
  theme_bw()+
...
# 
Seasonal_SOC_graph <- temp_graph1 / temp_graph2 +
  plot_layout(axes = "collect",
              axis_titles = "collect",
              heights = c(5,2))
